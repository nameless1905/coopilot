...
class Tools:
    ...
    async def plot_qc_data(
        ...
        """
        Выполняет агрегирующий SELECT (SUM/COUNT/AVG ... GROUP BY, и т.п.) и строит по
        результату график. Запрос должен возвращать УЖЕ агрегированные данные — одну
        колонку-категорию и одну-две числовые колонки, а не сырые строки таблицы.
        Пример хорошего запроса: SELECT status, COUNT(*) AS cnt FROM orders GROUP BY status.
        ...
        if not rows:
            return "Запрос вернул 0 строк — строить график не из чего."

        df = pd.DataFrame(rows, columns=cols)
        x = x_col or df.columns[0]
        y = y_col or (df.columns[1] if len(df.columns) > 1 else df.columns[0])
        if x not in df.columns or y not in df.columns:
            return f"Колонки '{x}'/'{y}' не найдены в результате. Доступны: {list(df.columns)}"
        ...
        img_html = (
            ...
            f'<img src="data:image/png;base64,{b64}" style="max-width:100%;height:auto;display:block;margin:0 auto;" />'
...
"""
        ...
        return f"![{title or chart_type}](data:image/png;base64,{b64})"